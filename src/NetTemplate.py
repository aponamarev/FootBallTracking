...
class NetTemplate(object):
    ...
    def _batch_norm(self, input, name=None, trainable=True):
        return tf.cond(tf.equal(self.is_training, tf.constant(True)),
                       lambda: batch_norm(input, trainable=trainable, is_training=True, fused=True, updates_collections=None, scope=name),
                       lambda: batch_norm(input, trainable=trainable, is_training=False, fused=True, reuse=True, updates_collections=None, scope=name))
